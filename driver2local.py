...
async def get_job_titles(query_q):
    ...
    harvests_db = client['test_db_keertan']
...
def check():
    ...
    print(conn)
    print(doc)
    hiturl = 'https://www.salary.com/research/salary/JobSalary/SalAjxGetJobsByKeyword'
    gen = master_check(hiturl, 'salary.com', 'POST', 5)
    next(gen)
    next(gen)
    n = gen.send([conn,doc])
    # print(n)
